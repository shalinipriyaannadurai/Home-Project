...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message {
    ...
    if ( self.authenticate == YES && self.opened == YES && self.followup == NO ) {
        if ( [message rangeOfString:@"error"].location == NSNotFound ) {
            NSString *json = [NSString stringWithFormat:@"{\"path\":\"/api/v1/device/perform/%@\",\"requestID\":\"%lu\",\"perform\":\"%@\",\"parameter\":%@}", self.device, (unsigned long)[Client sharedClient].requestCounter, self.request, self.parameters ];
            NSLog(@"json = %@", json);
...
- (void)webSocketDidOpen:(SRWebSocket *)webSocket {
    ...
    if (self.authenticate == YES && self.opened == NO ) {
        ...
        json = [NSString stringWithFormat:@"{\"path\":\"%@\",\"requestID\":\"%lu\",\"perform\":\"%@\",\"parameter\":%@}", self.device, (unsigned long)[Client sharedClient].requestCounter, self.request, self.parameters ];
    }
    NSLog(@"json = %@", json);
    [Client sharedClient].requestCounter = [Client sharedClient].requestCounter + 1;
    [webSocket send:json];
